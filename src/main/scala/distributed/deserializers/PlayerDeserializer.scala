...
object PlayerDeserializer extends JsonDeserializer[Player] {
  override def deserialize(json: JsonElement, typeOfT: Type, context: JsonDeserializationContext): Player = {
    ...
    PlayerImpl(jsonPlayer.get("userId").getAsInt, jsonPlayer.get("username").getAsString, jsonPlayer.get("idImage").getAsInt, position,jsonPlayer.get("isVisible").getAsBoolean, jsonPlayer.get("isFighting").getAsBoolean)
  }
}
